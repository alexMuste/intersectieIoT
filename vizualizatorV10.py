...
# Funcție pentru a adăuga pietoni dintr-un "server"
def adauga_pieton_din_server(direction):
    if direction == "up":
...
while running:
    ...
    for vehicle in masini:
        if stare_semafor == "roșu":
            if vehicle["direction"] == "left" and vehicle["x"] > 470 and vehicle["x"] < 475:  # Dacă mașina vine din dreapta și a ajuns aproape de trecerea de pietoni
                vehicle["stare_oprit"] = True
            elif vehicle["direction"] == "right" and vehicle["x"] > 240 and vehicle["x"] < 245:  # Dacă mașina vine din stânga și a ajuns aproape de trecerea de pietoni
                vehicle["stare_oprit"] = True
            else:
                vehicle["stare_oprit"] = False
        elif stare_semafor == "verde":
            for pedestrian in pietoni:
                if pedestrian["y"] > 200 and pedestrian["y"] < 350:
                    vehicle["stare_oprit"] = True
                else:
                    vehicle["stare_oprit"] = False

        # Deplasează vehiculul doar dacă nu este oprit
    ...
    # Funcția de generare a unui pieton nou
    def genereaza_pieton():
        direction = random.choice(["up", "down"])
        if direction == "up":
            return {"x": 400, "y": 370, "direction": "up", "is_crossing": False}
        elif direction == "down":
            return {"x": 430, "y": 180, "direction": "down", "is_crossing": False}
    ...
    # Adăugăm pietoni noi pentru a înlocui cei care au traversat
    while len(pietoni) < 2:  # Ne asigurăm că există întotdeauna 2 pietoni
        pietoni.append(genereaza_pieton())
